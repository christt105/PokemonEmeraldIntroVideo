...
`out/project.gif` and `out/project.webm` are regenerated by [`.github/workflows/export.yml`](.github/workflows/export.yml) on every push to `main` that touches anything outside `out/`: it checks out a `parallax-scene-editor` build, drives it headlessly through Playwright, and commits the result back. No need to open the editor and export by hand just to see whether a speed change looks right.

To run it locally instead:

```bash
npm install
EDITOR_DIR=/path/to/parallax-scene-editor npm run export
```
